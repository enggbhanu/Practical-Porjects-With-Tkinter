...
class BaseWindow:
    def __init__(self, title="GUI"):
        ...
        self.frm_browse_dir = tk.LabelFrame(self.frm_main)
        self.frm_browse_dir.grid(column=0, row=0, sticky='w')

        self.lbl_browse = ttk.Label(self.frm_browse_dir, text="Brows Source Image Directory..", relief=tk.SUNKEN)
        ...
        self.txt_dir_path = ttk.Entry(self.frm_browse_dir, textvariable=self.dir_path, width=25)
        self.txt_dir_path.grid(column=2, row=0, padx=2, sticky='e')

        self.lst_src_files = tk.Listbox(self.frm_browse_dir, selectmode=tk.MULTIPLE, width=25, height=8)
        self.lst_src_files.grid(column=0, row=1, padx=2, sticky='w')

        self.btn_convert = ttk.Button(self.frm_browse_dir, text="Convert", command=self.convert_image)
        self.btn_convert.grid(column=1, row=1, padx=5)

        self.lst_dst_files = tk.Listbox(self.frm_browse_dir, selectmode=tk.MULTIPLE, width=25, height=8)
        self.lst_dst_files.grid(column=2, row=1, padx=2, sticky='e')

        self.scrl_output_widget = scrolledtext.ScrolledText(self.base_win, wrap=tk.WORD, width=30)
